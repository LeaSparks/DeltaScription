# The ground itself cracks as a fountain of darkness spews night from the earth...

Slay them? Spare them?, that's up to you to decide.



This mod adds cards for Act 1 Kaycee's Mod in the game Inscryption, inspired by the Darkners and Lightners from Toby Fox's Hit game deltarune.



PLEASE reach out to me on the Modding Discord if you encounter any bugs, visual or otherwise, this is an early access mod.



# Installation

If you're installing the mod manually:

- click the **Manual Download** button to download the package.

- Unzip the contents of the package to a new folder in your **plugins** directory.

- Make sure that you have installed all the dependencies required by this mod as well.

If you're using a mod manager it should just work.



# Release Notes

v0.5.4:

- Published the Mod featuring:

	- Flower Kingdom  |26/26 Cards| Complete!
	- Dark Sanctuary  |26/26 Cards| Complete!
	- TV World        |??/?? Cards| N/A
	- Cyber World     |42/42 Cards| Testing Required!
	- Card Kingdom    |30/30 Cards| WIP
	- Hometown        |00/22 Cards| WIP
	- Weird Route     |02/?? Cards| WIP
	- Equipment       |16/60 Cards| WIP
	- Miscellaneous   |05/?? Cards| WIP
	- 16 new starter decks to try in this current state.



# Patch Notes

- Fixed Maddie's Background
- Added Chapter 1 Equipment Cards: NOT TEXTURED
- Fixed TailOfHell spawn trigger
- Fixed TailOfHell Background
- Attempted to fix name colour issue on AngelsHeaven Background


# Credits

- Lea - the main developer of the mod, put together the cards themselves and did the art assets for the project



## Special Thanks

- Nevernamed - For some custom sigil implementation used on Flowery as well as the entire Nevernamed Sigils Pack



# Card List



WIP, check the modding discord

